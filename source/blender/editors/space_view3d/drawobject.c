                       ...
                       const short dflag, const unsigned char ob_wire_col[4])
{
	...
	/* BKE_camera_model_matrix already accounts for scale, don't do it here */
...
static bool object_is_halo(Scene *scene, Object *ob)
{
	...
	return (ma && (ma->material_type == MA_TYPE_HALO));
}
                             ...
                             const char dt, const unsigned char ob_wire_col[4], const short dflag)
{
	...
	if (obedit && ob != obedit && ob->data == obedit->data) {
		if (ob->modifiers.first || obedit->modifiers.first) {}
	...
	if ((dflag & DRAW_PICKING) == 0 && (v3d->flag2 & V3D_RENDER_SHADOW) == 0) {
