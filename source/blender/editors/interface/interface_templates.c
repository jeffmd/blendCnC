        ...
        ModifierData *md, int index, int cageIndex, int lastCageIndex)
{
	...
	if (isVirtual) {
		...
		if (ob->type == OB_MESH) {
			...
			if (ELEM(md->type, eModifierType_Hook, eModifierType_MeshDeform)) {
        ...
        bool levels, bool neg_slope)
{
	...
	curvemap_buttons_layout(layout, &cptr, type, levels, neg_slope, cb);
...
void uiTemplateColormanagedViewSettings(uiLayout *layout, bContext *UNUSED(C), PointerRNA *ptr, const char *propname)
{
	...
	if (view_settings->flag & COLORMANAGE_VIEW_USE_CURVES)
		uiTemplateCurveMapping(col, &view_transform_ptr, "curve_mapping", 'c', false, false);
}
