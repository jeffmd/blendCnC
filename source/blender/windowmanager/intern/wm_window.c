...
void wm_window_title(wmWindowManager *wm, wmWindow *win)
{
	if (win->screen && win->screen->temp) {
		...
		if (G.save_over && BKE_main_blendfile_path_from_global()[0]) {
			...
			BLI_snprintf(str, sizeof(str), BLENDER_NAME"%s [%s%s]", wm->file_saved ? "" : "*",
			             BKE_main_blendfile_path_from_global(),
			             G_MAIN->recovered ? " (Recovered)" : "");
			GHOST_SetTitle(win->ghostwin, str);
		}
		else
			GHOST_SetTitle(win->ghostwin, BLENDER_NAME);
...
void wm_window_ghostwindows_ensure(wmWindowManager *wm)
{
	...
	for (win = wm->windows.first; win; win = win->next) {
		if (win->ghostwin == NULL) {
			...
			wm_window_ghostwindow_add(wm, BLENDER_NAME, win);
		}
...
wmWindow *WM_window_open_temp(bContext *C, int x, int y, int sizex, int sizey, int type)
{
	...
	if (sa->spacetype == SPACE_IMAGE)
		title = IFACE_(BLENDER_NAME" Render");
	else if (ELEM(sa->spacetype, SPACE_OUTLINER, SPACE_USERPREF))
		title = IFACE_(BLENDER_NAME" User Preferences");
	else if (sa->spacetype == SPACE_FILE)
		title = IFACE_(BLENDER_NAME" File View");
	else
		title = BLENDER_NAME;
