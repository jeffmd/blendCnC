...
void curve_position_weights(float t, float data[4], int type)
{
	float t2, t3, fc;
...
/* first derivative */
void curve_tangent_weights(float t, float data[4], int type)
{
	float t2, fc;
...
/* second derivative */
void curve_normal_weights(float t, float data[4], int type)
{
	float fc;
		...
		if (tilt_array) {
			if (nu->tilt_interp == KEY_CU_EASE) { /* May as well support for tilt also 2.47 ease interp */
				*tilt_array = prevbezt->alfa +
				...
				curve_position_weights(fac, t, nu->tilt_interp);
			...
			if (nu->radius_interp == KEY_CU_EASE) {
				...
				*radius_array = prevbezt->radius +
				...
				if (tilt_array == NULL || nu->tilt_interp != nu->radius_interp) {
					curve_position_weights(fac, t, nu->radius_interp);
				}
				*radius_array =
...
static void calchandlesNurb_intern(Nurb *nu, bool skip_align)
{
	...
	while (a--) {
		calchandleNurb_intern(bezt, prev, next, skip_align);
		prev = bezt;
		if (a == 1) {
			if (nu->flagu & CU_NURB_CYCLIC)
				next = nu->bezt;
...
{
	calchandleNurb_intern(bezt, prev, next, false);
}
 ...
 */
void BKE_curve_calc_path(Object *ob, ListBase *nurbs)
{
	BevList *bl;
...
int BKE_curve_where_on_path(Object *ob, float ctime, float vec[4], float dir[3])
{
	...
	if ((nu->type == CU_POLY) || (nu->type == CU_BEZIER)) key = KEY_LINEAR;
	else if (s0 == s1 || p2 == p3) key = KEY_CARDINAL;

	curve_position_weights(1.0f - fac, data, key);
